...
public class PaymentsController {
    ...
    @PostMapping("/init")
    public Mono<Void> initCancel(@RequestBody CancelPaymentRequestDTO dto) {
        return orderService.initCancel(dto.getMerchantUid());
    }

    // 2) 취소 확정: Redis 키 삭제
    @PostMapping("/confirm")
    public Mono<Void> confirmCancel(@RequestBody CancelPaymentRequestDTO dto) {
        return orderService.confirmCancel(dto.getMerchantUid());
    }

    // 3) 보상 트랜잭션: Redis 에 남아 있던 이전 상태로 롤백
    @PostMapping("/compensate")
    public Mono<Void> compensateCancel(@RequestBody CancelPaymentRequestDTO dto) {
